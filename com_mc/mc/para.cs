...
namespace com_mc
{
	...
	public class ParaValue //参数父类(无处理二进制块)
	{
		...
		public virtual void from_json(Dictionary<string, object> v) { } //从json构造对象
		...
		public virtual int set_val(byte[] b, int off,int n) //从数据设定值
		{
		...
		public virtual int get_val(byte[] b, int off,int n) //向数据缓存中复制数据
		{
			for (int i = 0; i < len && i < data.Length && i<n && off<b.Length; i++)
			{
				b[off] = data[i]; off++;
	...
	public class ParaValue_Val : ParaValue //值类型
	{
		...
		public override string ToString()
		{
			switch (type)
			{
				case DestType.str: return "type err";
				case DestType.u:
					break;
				case DestType.s:
					break;
